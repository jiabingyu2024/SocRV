...
(
   ...
   output logic        target_misaligned, // Taken control-flow target violates IALIGN=32
   ...
   // This derivative has no C extension, so every architectural instruction
   // address must be 4-byte aligned.  Report the bad target independently of
   // branch prediction: even a correctly predicted taken branch must trap.
