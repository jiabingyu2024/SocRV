...
`endif
   ...
   // EH1F Stage-B implements IALIGN=32. The second issued instruction is
   // therefore always the next 32-bit word.
   ...
   // Cache/TCM error checking is absent from the no-ECC derivative.
`ifdef RV_TCM_ONLY
...
`endif // RV_TCM_ONLY
   ...
   // Compressed trace payloads are architecturally absent in RV32IMF.
   assign ifu_i0_cinst[15:0] = 16'b0;
   assign ifu_i1_cinst[15:0] = 16'b0;
   // end trace

   // RV32IMF has no C extension: every instruction consumes four bytes.
   // Keeping these local names during Stage-B limits churn in the branch
   // metadata queue; synthesis removes every former 16-bit path.
   assign first4B  = 1'b1;
   assign first2B  = 1'b0;
...
// 16-bit encodings are never independently decoded. A word whose opcode is
// not a legal 32-bit instruction is rejected by the normal decoder.
   ...
   // Former decompressor outputs remain locally tied during the staged
   // refactor so packet interfaces do not change in the same edit.
