...
`include "global.h"
   ...
   // The frozen software contract selects trap-on-misaligned behavior for all
   // halfword/word data accesses.  The inherited EH1 DCCM datapath can merge a
   // split access, but that optional behavior is deliberately disabled here so
   // DCCM and MMIO expose one precise architectural rule.
   assign misaligned_fault_dc1 = lsu_pkt_dc1.valid && !is_aligned_dc1;
