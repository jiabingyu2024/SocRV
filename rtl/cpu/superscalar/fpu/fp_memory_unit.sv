...
module fp_memory_unit (
    ...
    typedef enum logic [1:0] {
        FP_MEM_IDLE, FP_MEM_REQ, FP_MEM_WAIT
    } state_e;
    ...
    assign misaligned_c = |req_addr_c[1:0];

    always_comb begin
        cache_req_valid_o = state_q == FP_MEM_REQ;
        cache_req_write_o = req_q.uop.fp_op == FP_STORE;
        cache_req_addr_o = addr_q;
        cache_req_wdata_o = req_q.fp_op2;
            ...
            unique case (state_q)
                ...
                FP_MEM_WAIT: begin
                    if (cache_resp_valid_i) begin
                        ...
                        completion_q.valid <= 1'b1;
                        completion_q.trans_id <= req_q.trans_id;
                        completion_q.fp_result <= cache_resp_rdata_i;
