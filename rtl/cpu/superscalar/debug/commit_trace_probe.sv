...
module commit_trace_probe (
    ...
    input  logic [31:0] issue_src1_i,
    input  logic [31:0] issue_src2_i,
    ...
    input  logic [1:0] csr_privilege_i,
    ...
    output logic [4:0] commit_rs1_o,
    output logic [31:0] commit_rs1_rdata_o,
    output logic [4:0] commit_rs2_o,
    ...
    mem_size_e mem_size_q [0:SCOREBOARD_DEPTH-1];
    logic [31:0] next_pc_q [0:SCOREBOARD_DEPTH-1];
    logic [4:0] rs1_q [0:SCOREBOARD_DEPTH-1];
    logic [31:0] rs1_rdata_q [0:SCOREBOARD_DEPTH-1];
    logic [4:0] rs2_q [0:SCOREBOARD_DEPTH-1];
    logic [31:0] rs2_rdata_q [0:SCOREBOARD_DEPTH-1];
    integer i;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            commit_next_pc_o <= '0;
            commit_mem_addr_o <= '0;
            commit_mem_rmask_o <= '0;
            commit_mem_rdata_o <= '0;
            commit_mem_wdata_o <= '0;
            commit_mem_wstrb_o <= '0;
            commit_rs1_o <= '0;
            commit_rs1_rdata_o <= '0;
            commit_rs2_o <= '0;
            commit_rs2_rdata_o <= '0;
            commit_tval_o <= '0;
            commit_mode_o <= '0;
            commit_csr_mstatus_o <= '0;
            commit_csr_mie_o <= '0;
            commit_csr_mip_o <= '0;
            commit_csr_mtvec_o <= '0;
            commit_csr_mscratch_o <= '0;
            commit_csr_mepc_o <= '0;
            commit_csr_mcause_o <= '0;
            commit_csr_mtval_o <= '0;
            commit_csr_mcycle_o <= '0;
            commit_csr_minstret_o <= '0;
            for (i = 0; i < SCOREBOARD_DEPTH; i = i + 1) begin
                mem_addr_q[i] <= '0;
                mem_wdata_q[i] <= '0;
                mem_wstrb_q[i] <= '0;
                mem_size_q[i] <= MEM_WORD;
                next_pc_q[i] <= '0;
                rs1_q[i] <= '0;
                rs1_rdata_q[i] <= '0;
                rs2_q[i] <= '0;
            ...
            if (commit_i) begin
                ...
                commit_cause_o <= trap_cause_i;
                commit_tval_o <= commit_entry_i.exception_tval;
                ...
                commit_mem_rmask_o <= '0;
                commit_mem_rdata_o <= '0;
                if (commit_entry_i.fu == FU_LOAD) begin
                    unique case (mem_size_q[commit_trans_id_i])
                        MEM_BYTE: begin
                            commit_mem_rmask_o <= 4'b0001 <<
                                mem_addr_q[commit_trans_id_i][1:0];
                            commit_mem_rdata_o <=
                                {24'd0, commit_entry_i.result[7:0]} <<
                                {mem_addr_q[commit_trans_id_i][1:0], 3'b000};
                        end
                        MEM_HALF: begin
                            commit_mem_rmask_o <= 4'b0011 <<
                                mem_addr_q[commit_trans_id_i][1:0];
                            commit_mem_rdata_o <=
                                {16'd0, commit_entry_i.result[15:0]} <<
                                {mem_addr_q[commit_trans_id_i][1:0], 3'b000};
                        end
                        default: begin
                            commit_mem_rmask_o <= 4'b1111;
                            commit_mem_rdata_o <= commit_entry_i.result;
                        end
                    endcase
                end
                commit_rs1_o <= rs1_q[commit_trans_id_i];
                commit_rs1_rdata_o <= rs1_rdata_q[commit_trans_id_i];
                commit_rs2_o <= rs2_q[commit_trans_id_i];
                commit_rs2_rdata_o <= rs2_rdata_q[commit_trans_id_i];
                commit_mode_o <= csr_privilege_i;
                commit_csr_mstatus_o <= csr_mstatus_i;
                commit_csr_mie_o <= csr_mie_i;
                commit_csr_mip_o <= csr_mip_i;
                commit_csr_mtvec_o <= csr_mtvec_i;
                commit_csr_mscratch_o <= csr_mscratch_i;
                commit_csr_mepc_o <= csr_mepc_i;
                commit_csr_mcause_o <= csr_mcause_i;
                commit_csr_mtval_o <= csr_mtval_i;
                commit_csr_mcycle_o <= cycle_i;
                commit_csr_minstret_o <= commit_count_i;
                if (commit_entry_i.exception_valid)
                    commit_next_pc_o <= {csr_mtvec_i[31:2], 2'b00};
            ...
            if (issue_i) begin
                ...
                rs1_q[issue_trans_id_i] <= issue_uop_i.rs1;
                rs1_rdata_q[issue_trans_id_i] <= issue_src1_i;
                rs2_q[issue_trans_id_i] <= issue_uop_i.rs2;
                rs2_rdata_q[issue_trans_id_i] <= issue_src2_i;
                mem_size_q[issue_trans_id_i] <= issue_uop_i.mem_size;
            end
            if (branch_resolve_i)
            ...
            if (exec_i.valid && (exec_i.uop.fu == FU_LOAD ||
                ...
                mem_wdata_q[exec_i.trans_id] <= exec_i.uop.fu == FU_STORE ?
                    exec_i.op2 << {exec_mem_addr_i[1:0], 3'b000} : 32'd0;
                if (exec_i.uop.fu == FU_STORE) begin
                    unique case (exec_i.uop.mem_size)
                        MEM_BYTE: mem_wstrb_q[exec_i.trans_id] <=
                            4'b0001 << exec_mem_addr_i[1:0];
                        MEM_HALF: mem_wstrb_q[exec_i.trans_id] <=
                            4'b0011 << exec_mem_addr_i[1:0];
