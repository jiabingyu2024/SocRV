...
module hazard_unit(
    ...
    always_comb begin
        ...
        // HXI fetch responses are variable-latency.  Hold only the PC while
        // no instruction is available; IF/ID is allowed to consume its old
        // instruction once and then captures the zero-valued bubble emitted
        // by stage_if.  This keeps older pipeline stages draining instead of
        // turning instruction-bus latency into a whole-core clock enable.
        o_stall_p_f = data_hazard || i_m_busy || i_mem_busy || !i_fetch_valid;
        ...
        end else if (data_hazard || i_m_busy || i_mem_busy || !i_fetch_valid) begin
            o_pc_next = i_pc_cur;
        end else begin
