...
module core #(
    ...
    logic             f_hazard_d;
    cpu_types_pkg::commit_trace_t trace_ex;
    cpu_types_pkg::commit_trace_t trace_m;
    cpu_types_pkg::commit_trace_t trace_m2;
    cpu_types_pkg::commit_trace_t trace_w;
    logic [63:0] commit_order_q;

    assign irom_addr = pc_p;
    // The target SoC instruction port is ready/valid rather than a fixed
    // one-cycle ROM.  Keep a request active for the current PC; the adapter
    // suppresses duplicates while one HXI transaction is outstanding.
    assign irom_ena   = rst_n;
    assign irom_ready = !stall_f_d || flush_f_d;
    ...
    stage_if u_stage_if (
        ...
        .i_valid      (irom_valid && irom_ready && !flush_f_d),
        .o_pc         (pc_f),
        .o_inst       (inst_f),
    ...
    // Trace metadata follows the same enables and flushes as the architectural
    // datapath.  It is intentionally registered at every existing stage so
    // enabling DiffTest/ILA does not lengthen execute or memory timing cones.
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (flush_e_m) trace_m <= '0;
            else if (!stall_e_m) trace_m <= trace_ex;

            if (flush_m_w) trace_m2 <= '0;
            else if (!stall_m_w) trace_m2 <= trace_m;

            if (!stall_m_w) trace_w <= trace_m2;
            if (trace_w.valid || trace_w.irq_valid)
    ...
    always_comb begin
        ...
        commit_o.irq_next_order = commit_order_q;
        if (trace_w.rd_wen) commit_o.rd_wdata = wb_data_arch;
        if (trace_w.mem_valid && (trace_w.mem_rmask != 4'b0)) begin
            commit_o.mem_rdata = wb_data_arch <<
                                 {trace_w.mem_addr[1:0], 3'b000};
        end
    end
