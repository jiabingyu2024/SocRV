...
module stage_ex #(
    ...
    ,output cpu_types_pkg::commit_trace_t    o_trace
);
    ...
    branch_cmp u_branch_cmp (
        ...
        .o_update_taken  (branch_update_taken),
        .o_update_en     (branch_update_en),
        .o_update_pc     (branch_update_pc),
    ...
    logic csr_access_read_only;
    logic [1:0] csr_privilege;
    logic [31:0] ecall_cause;
    ...
    assign trap_illegal = valid_q && (illegal_q ||
                          (is_mret && (csr_privilege != 2'b11)) ||
                          (is_csr_inst &&
                           (!csr_access_valid ||
                            (csr_privilege < csr_addr_q[9:8]) ||
                            (csr_we && csr_access_read_only))));
    assign trap_inst_misaligned = branch_update_taken &&
                                  (branch_update_target[1:0] != 2'b00);
    assign trap_interrupt = interrupt_pending && valid_q;
    assign trap_any = trap_interrupt || trap_illegal ||
                      trap_inst_misaligned || is_ecall || is_ebreak;
    always_comb begin
        trap_cause = 32'd0;
        trap_tval = 32'd0;
        if (trap_interrupt) begin
            trap_cause = interrupt_cause;
        end else if (trap_illegal) begin
            trap_cause = 32'd2;
            trap_tval = instr_q;
        end else if (trap_inst_misaligned) begin
            trap_cause = 32'd0;
            trap_tval = branch_update_target;
    ...
    assign retire_event = valid_q && !i_stall_e &&
                          !o_m_busy && !trap_any;
    ...
    csr_file u_csr_file (
        ...
        .i_raddr      (csr_addr_q),
        .o_rdata      (csr_rdata),
        .o_access_valid(csr_access_valid),
        .o_access_read_only(csr_access_read_only),
        // 写
        .i_we         (csr_we && !trap_any),
        .i_waddr      (csr_addr_q),
        .i_wdata      (csr_operand),
        .i_wmode      (csr_wmode),
        ...
        // Trap 返回
        .i_mret_en    (is_mret && !trap_illegal),
        .i_retire     (retire_event),
        .i_irq_software(i_irq_software),
        .i_irq_timer  (i_irq_timer),
        .i_irq_external(i_irq_external),
        .o_interrupt_pending(interrupt_pending),
        .o_interrupt_cause(interrupt_cause),
        .o_privilege  (csr_privilege),
        .o_ecall_cause(ecall_cause),
        // 直接输出
        .o_mtvec      (mtvec_val),
        .o_mepc       (mepc_val),
        ...
        .o_mie        (csr_mie),
        .o_mip        (csr_mip),
        .o_mscratch   (csr_mscratch),
        .o_mcause     (csr_mcause),
        .o_mtval      (csr_mtval),
        .o_mcycle     (csr_mcycle),
        .o_minstret   (csr_minstret)
    );
    ...
    always_comb begin
        if (trap_any) begin
            o_error     = 1'b1;
            if (trap_interrupt && mtvec_val[0])
                o_update_target = {mtvec_val[31:2], 2'b00} +
                                  {2'b00, trap_cause[27:0], 2'b00};
            else
                o_update_target = {mtvec_val[31:2], 2'b00};
        end else if (is_mret && !trap_illegal) begin
            o_error     = 1'b1;
    ...
    assign o_mem_read      = mem_read_q && !trap_any;
    ...
    assign o_update_en = branch_update_en && !trap_any;
    assign o_update_pc = branch_update_pc;
    ...
    always_comb begin
        o_trace = '0;
        o_trace.valid       = valid_q && !trap_interrupt;
        o_trace.retired     = valid_q && !trap_any;
        o_trace.pc_rdata    = pc_q;
        o_trace.pc_wdata    = o_update_target;
        o_trace.instruction = instr_q;
        o_trace.rs1_addr    = rs1_addr_q;
        o_trace.rs1_rdata   = rs1_exec_registered;
        o_trace.rs2_addr    = rs2_addr_q;
        o_trace.rs2_rdata   = rs2_exec_registered;
        o_trace.rd_wen      = reg_write_q && !trap_any;
        o_trace.rd_addr     = rd_addr_q;
        o_trace.rd_wdata    = o_alu_res;
        o_trace.sync_trap   = valid_q && trap_any && !trap_interrupt;
        o_trace.cause       = trap_cause;
        o_trace.tval        = trap_tval;
        o_trace.mode        = csr_privilege;
        o_trace.mem_valid   = valid_q && !trap_any &&
                              (mem_read_q || mem_write_q);
        o_trace.mem_addr    = o_mem_addr;
        o_trace.mem_rmask   = mem_read_q
            ? ((mem_mask_q << o_mem_addr[1:0]) & 4'hf) : 4'h0;
        o_trace.mem_wmask   = mem_write_q
            ? ((mem_mask_q << o_mem_addr[1:0]) & 4'hf) : 4'h0;
        o_trace.mem_wdata   = o_a2_data << {o_mem_addr[1:0], 3'b000};
        o_trace.csr_mstatus = csr_mstatus;
        o_trace.csr_mie     = csr_mie;
        o_trace.csr_mip     = csr_mip;
        o_trace.csr_mtvec   = mtvec_val;
        o_trace.csr_mscratch = csr_mscratch;
        o_trace.csr_mepc    = mepc_val;
        o_trace.csr_mcause  = csr_mcause;
        o_trace.csr_mtval   = csr_mtval;
        o_trace.csr_mcycle  = csr_mcycle;
        o_trace.csr_minstret = csr_minstret;
        o_trace.irq_valid   = trap_interrupt;
        o_trace.irq_mip_pre = csr_mip;
        o_trace.irq_mip_post = csr_mip;
    end
