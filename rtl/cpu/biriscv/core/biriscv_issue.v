...
(
    ...
    ,output [ 31:0]  commit0_pc_o
    ,output [ 31:0]  commit0_next_pc_o
    ,output [ 31:0]  commit0_opcode_o
    ,output [  4:0]  commit0_rd_o
    ,output [ 31:0]  commit0_result_o
    ,output [ 31:0]  commit0_ra_value_o
    ,output [ 31:0]  commit0_rb_value_o
    ,output [  5:0]  commit0_exception_o
...
// Architectural completion export used by the SocRV wrapper. Slot 0 carries
// the preliminary trace payload; slot 1 feeds the dual-issue retire counter
// until a second full architectural trace is integrated.
// A faulting instruction has its architectural payload in WB even though
// valid_wb_o suppresses register writeback.  Keep it visible as a commit event
// so the SocRV trap checker can observe the exception and PC.
assign commit0_valid_o     = pipe0_valid_wb_w | (|pipe0_exception_wb_w);
assign commit0_pc_o        = pipe0_pc_wb_w;
assign commit0_next_pc_o   = pipe0_npc_wb_w;
assign commit0_opcode_o    = pipe0_opc_wb_w;
assign commit0_rd_o        = pipe0_rd_wb_w;
assign commit0_result_o    = pipe0_result_wb_w;
assign commit0_ra_value_o  = pipe0_ra_val_wb_w;
assign commit0_rb_value_o  = pipe0_rb_val_wb_w;
assign commit0_exception_o = pipe0_exception_wb_w;
assign commit1_valid_o     = pipe1_valid_wb_w;
