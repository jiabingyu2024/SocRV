...
// HXI provides a single 32-bit request/response transaction.  A cache-line
// refill is therefore serialized into eight word reads.  Only one request is
// outstanding, matching the SocRV CPU-master contract.
reg         hxi_outstanding_q;
reg         refill_error_q;
wire        hxi_req_fire_w = hxi_req_valid_o && hxi_req_ready_i;
wire        hxi_rsp_fire_w = hxi_rsp_valid_i && hxi_rsp_ready_o;
wire        refill_last_w  = (refill_word_idx_q == 3'd7);
wire        refill_ok_w    = !hxi_rsp_err_i && !refill_error_q;
...
begin
    ...
    // Line refill
        ...
        tag0_write_r = hxi_rsp_fire_w && refill_last_w &&
                       refill_ok_w && (replace_way_q == 0);
end
...
begin
    ...
    // Line refill
        ...
        tag1_write_r = hxi_rsp_fire_w && refill_last_w &&
                       refill_ok_w && (replace_way_q == 1);
end
...
else if (hxi_rsp_fire_w && refill_last_w)
    refill_word_idx_q <= 3'b0;
else if (hxi_rsp_fire_w)
...
if (rst_i)
    ...
    refill_lower_q <= hxi_rsp_rdata_i;
...
if (rst_i)
    ...
    data_write_addr_q <= {lookup_addr_q[`ICACHE_TAG_REQ_RNG],
                          {(ICACHE_LINE_SIZE_W-3){1'b0}}};
else if (state_q == STATE_REFILL && hxi_rsp_fire_w && refill_word_idx_q[0])
...
always @ *
begin
    data0_write_r = hxi_rsp_fire_w && refill_word_idx_q[0] &&
                    !hxi_rsp_err_i && (replace_way_q == 0);
end
...
(
  ...
  .rst_i(rst_i),
  .addr_i(data_addr_r),
  .data_i({hxi_rsp_rdata_i, refill_lower_q}),
  .wr_i(data0_write_r),
...
always @ *
begin
    data1_write_r = hxi_rsp_fire_w && refill_word_idx_q[0] &&
                    !hxi_rsp_err_i && (replace_way_q == 1);
end
...
// Instruction Output
//-----------------------------------------------------------------
assign req_valid_o = lookup_valid_q && ((state_q == STATE_LOOKUP) ?
                                        (tag_hit_any_w || hxi_error_q) : 1'b0);
...
begin
    ...
    begin
        ...
        if (lookup_valid_q && !hxi_error_q && !tag_hit_any_w)
    ...
    begin
        // End of refill
        if (hxi_rsp_fire_w && refill_last_w)
...
// Remember any failed word in the current line.  The line is only tagged
// valid when all eight HXI responses succeeded.
always @ (posedge clk_i or posedge rst_i)
if (rst_i)
    refill_error_q <= 1'b0;
else if (state_q != STATE_REFILL && next_state_r == STATE_REFILL)
    refill_error_q <= 1'b0;
else if (hxi_rsp_fire_w && hxi_rsp_err_i)
    refill_error_q <= 1'b1;

//-----------------------------------------------------------------
// HXI request tracking
//-----------------------------------------------------------------
always @ (posedge clk_i or posedge rst_i)
if (rst_i)
    hxi_outstanding_q <= 1'b0;
else if (hxi_req_fire_w)
    hxi_outstanding_q <= 1'b1;
else if (hxi_rsp_fire_w)
    hxi_outstanding_q <= 1'b0;
...
if (rst_i)
    ...
    hxi_error_q   <= 1'b1;
else if ((req_rd_i && req_accept_o) || req_valid_o)
    hxi_error_q   <= 1'b0;

assign req_error_o = hxi_error_q;

//-----------------------------------------------------------------
// HXI line refill
//-----------------------------------------------------------------
assign hxi_req_valid_o = (state_q == STATE_REFILL) && !hxi_outstanding_q;
assign hxi_req_addr_o  = {lookup_addr_q[31:ICACHE_LINE_SIZE_W],
                          {(ICACHE_LINE_SIZE_W){1'b0}}} +
                         {27'b0, refill_word_idx_q, 2'b00};
assign hxi_rsp_ready_o = (state_q == STATE_REFILL) && hxi_outstanding_q;
