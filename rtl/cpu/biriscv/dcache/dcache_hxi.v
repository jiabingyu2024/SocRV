...
// presented one word at a time by dcache_core; each word is serialized onto
// HXI and the cache sees one completion only after the final write response.
//-----------------------------------------------------------------
module dcache_hxi
(
     input           clk_i
...
reg [31:0] wdata_q;
reg [3:0]  wstrb_q;
reg [7:0]  read_beats_left_q;
reg [7:0]  write_beats_left_q;
reg        write_error_q;
...
assign inport_ack_o       = rsp_fire_w &&
                            (!write_q || (write_beats_left_q == 8'd0));
assign inport_error_o     = inport_ack_o &&
                            (hxi_rsp_err_i || write_error_q);
...
begin
    ...
    wdata_q      <= 32'b0;
    wstrb_q      <= 4'b0;
    read_beats_left_q  <= 8'b0;
    write_beats_left_q <= 8'b0;
    write_error_q      <= 1'b0;
end
else
begin
    ...
    begin
        ...
        begin
            ...
            if (inport_rd_i)
                read_beats_left_q <= inport_len_i;
            else if (write_beats_left_q == 8'd0)
            begin
                write_beats_left_q <= inport_len_i;
                write_error_q      <= 1'b0;
            end
            else
                write_beats_left_q <= write_beats_left_q - 8'd1;
        end
    end
    STATE_REQ:
    begin
    ...
    begin
        if (rsp_fire_w)
        begin
            if (!write_q && (read_beats_left_q != 8'd0))
            begin
                state_q          <= STATE_REQ;
                addr_q           <= addr_q + 32'd4;
                read_beats_left_q <= read_beats_left_q - 8'd1;
            end
            else
                state_q <= STATE_IDLE;
