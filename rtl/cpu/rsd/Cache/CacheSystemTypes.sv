...
package CacheSystemTypes;
    ...
    typedef struct packed   // DCachePortMultiplexerIn
    {
        ...
        // Cached requests are matched at cache-line granularity.  SocRV MMIO
        // requests are uncached 32-bit transfers, so DCache must retain the
        // word selector to avoid bypassing data from the adjacent register.
        logic wordOffset;
        logic isUncachable;
