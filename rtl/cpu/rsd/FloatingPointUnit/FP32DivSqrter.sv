...
output
    ...
    FFlags_Path regFFlags, nextFFlags;
    ...
    wire lhs_is_snan = lhs_is_nan & !lhs_mant[22];
    wire rhs_is_snan = rhs_is_nan & !rhs_mant[22];
    ...
    wire       res_is_inf   = regData.is_divide ? $signed(regData.virtual_expo) >= 255 | regData.res_is_inf | result_expo == 8'hff
                                                : regData.res_is_inf;
    wire       finite_overflow = res_is_inf && !regData.res_is_inf &&
                                 !regData.res_is_nan;
    wire       rounding_inexact = (before_round[24:0] != 0) ||
    ...
    always_comb begin
        ...
        nextFFlags = regFFlags;
        if (req && regPhase == PHASE_FINISHED) begin
            nextFFlags = '0;
            nextFFlags.NV = lhs_is_snan | rhs_is_snan |
                            (is_divide && ((lhs_is_zero && rhs_is_zero) ||
                                           (lhs_is_inf && rhs_is_inf))) |
                            (!is_divide && lhs_is_neg);
            nextFFlags.DZ = is_divide && rhs_is_zero && !lhs_is_zero &&
                            !lhs_is_inf && !lhs_is_nan;
        ...
        // Here, quo has a <1/3ULP error.
            ...
            nextFFlags.OF = finite_overflow;
            nextFFlags.UF = regData.subnormal && rounding_inexact &&
                            !regData.res_is_nan;
            nextFFlags.NX = (rounding_inexact || finite_overflow) &&
                            !regData.res_is_nan;
