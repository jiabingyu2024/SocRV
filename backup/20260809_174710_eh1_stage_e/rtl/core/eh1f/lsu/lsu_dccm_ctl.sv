...
`ifdef RV_TCM_ONLY
   ...
   localparam int PIC_BITS = `RV_PIC_BITS;
   ...
   assign dccm_dma_ecc_error   = 1'b0;
   assign dccm_dma_rdata[63:0] = lsu_pkt_dc3.dword ? lsu_rdata_dc3[63:0] :
                                                       {2{lsu_rdata_dc3[31:0]}};
   ...
   assign picm_rd_data_dc3[63:0]     = {picm_rd_data_lo_dc3, picm_rd_data_lo_dc3};
   ...
   for (genvar noecc_byte = 0; noecc_byte < 8; noecc_byte++) begin: no_ecc_load_forward
      assign lsu_rdata_dc3[(8*noecc_byte)+7:(8*noecc_byte)] =
               ...
               addr_in_pic_dc3                 ? picm_rd_data_dc3[(8*noecc_byte)+7:(8*noecc_byte)] :
   ...
   // Partial or unaligned stores read the old word for byte merging.  Aligned
   // word/dword stores already provide every byte and do not consume a read port.
   assign lsu_dccm_rden_dc1 = lsu_pkt_dc1.valid &
                               (lsu_pkt_dc1.load |
                                (lsu_pkt_dc1.store &
                                 (~(lsu_pkt_dc1.word | lsu_pkt_dc1.dword) |
   ...
   assign picm_addr[31:0] = (picm_rden | picm_mken) ?
                              (`RV_PIC_BASE_ADDR | {17'b0, lsu_addr_dc1[14:0]}) :
                              (`RV_PIC_BASE_ADDR | {{32-PIC_BITS{1'b0}},
                                                    stbuf_addr_any[`RV_PIC_BITS-1:0]});
