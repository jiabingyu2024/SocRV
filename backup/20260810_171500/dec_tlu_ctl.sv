...
(
   ...
   input logic exu_i0_inst_misaligned_e4,   // pipe 0 taken target violates IALIGN=32
   input logic exu_i1_inst_misaligned_e4,   // pipe 1 taken target violates IALIGN=32
   input logic [31:1] exu_i0_inst_misaligned_addr_e4, // pipe 0 faulting target
   input logic [31:1] exu_i1_inst_misaligned_addr_e4, // pipe 1 faulting target
...
`endif
   ...
   //  Final commit valids
   // With IALIGN=32, a taken branch/JAL/JALR whose target has bit 1 set
   // raises instruction-address-misaligned (cause 0) on the control-flow
   // instruction itself.  Slot 0 has architectural priority over slot 1.
...
`endif
   ...
   //  [8]     I    - RV32I
   //  [2]     C    - zero; compressed extension is not implemented
   `define MISA 12'h301
   ...
   // RV32F architectural state. FS is Off after reset; startup software must
   // set it to Initial/Clean before executing an FP instruction.
...
`endif
   ...
   assign wr_meicurpl_wb = 1'b0;
   assign meicurpl_ns = '0;
   assign meicurpl = '0;
   assign dec_tlu_meicurpl = '0;
   ...
   assign wr_meicidpl_wb = 1'b0;
   assign meicidpl_ns = '0;
   assign meicidpl = '0;
   ...
   assign wr_meipt_wb = 1'b0;
   assign meipt_ns = '0;
   assign meipt = '0;
   assign dec_tlu_meipt = '0;
   // ----------------------------------------------------------------------
...
`ifdef RV_TCM_ONLY
   assign dicawics_ns = '0;
   assign dicawics = '0;
   assign dicad0_ns = '0;
   assign dicad0 = '0;
   assign dicad1_ns = '0;
   assign dicad1 = '0;
   assign wr_dicawics_wb = 1'b0;
   assign wr_dicad0_wb = 1'b0;
   assign wr_dicad1_wb = 1'b0;
   assign dec_tlu_ic_diag_pkt = '0;
`else
...
assign csr_meihap = 1'b0;

assign csr_meivt = 1'b0;

assign csr_meipt = 1'b0;

assign csr_meicurpl = 1'b0;

assign csr_meicidpl = 1'b0;
...
`ifdef RV_TCM_ONLY
assign csr_dicawics = 1'b0;
assign csr_dicad0   = 1'b0;
assign csr_dicad1   = 1'b0;
`else
...
assign dec_csr_legal_d = ( dec_csr_any_unq_d &
   ...
   // CSR read mux
  // RV32IMF_Zicsr, with C permanently clear.
  // The inherited CSR selects are Boolean-minimized over EH1's original
  // machine/debug CSR set.  Low-numbered F CSRs were not in that set and can
  // alias those minimized selects (for example, fflags also selects misa).
  // Give the architectural F CSRs exact-address priority before the legacy OR
  // mux so their read values cannot be contaminated by an alias.
