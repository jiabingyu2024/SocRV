...
日期：2026-08-06  
状态：RTL、Spike DiffTest、SoC/RT-Thread/CoreMark 仿真及 FPGA 实现已通过；等待实体板串口验收。
...
GPIO、TestStatus、SoC top 和 FPGA top 均未为新 CPU 改动。`data/soc/software_contract.json`
仅同步了 CPU 实现名、状态和已完成 gate 三个描述字段，没有修改地址、ISA 编译参数、
时钟、中断或外设合同。
