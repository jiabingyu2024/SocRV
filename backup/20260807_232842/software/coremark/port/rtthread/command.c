...
static int cmd_coremark(int argc, char **argv)
{
    ...
    uart_puts("\nSocRV HPM load misses: ");
    print_u64(coremark_hpm_delta(0u));
    uart_puts("\nSocRV HPM store misses: ");
    print_u64(coremark_hpm_delta(1u));
    uart_puts("\nSocRV HPM ICache misses: ");
    print_u64(coremark_hpm_delta(2u));
    uart_puts("\nSocRV HPM branch mispredicts: ");
    print_u64(coremark_hpm_delta(3u));
    uart_putc('\n');
