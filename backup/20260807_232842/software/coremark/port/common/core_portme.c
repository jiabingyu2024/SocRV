...
static ee_u32 start_hpm[4];
static ee_u32 stop_hpm[4];
static int data_error_seen;

static void read_coremark_hpm(ee_u32 value[4])
{
    __asm__ volatile ("csrr %0, mhpmcounter3" : "=r" (value[0]));
    __asm__ volatile ("csrr %0, mhpmcounter4" : "=r" (value[1]));
    __asm__ volatile ("csrr %0, mhpmcounter5" : "=r" (value[2]));
    __asm__ volatile ("csrr %0, mhpmcounter6" : "=r" (value[3]));
}
...
void start_time(void)
{
    ...
    read_coremark_hpm(start_hpm);
}

void stop_time(void)
{
    read_coremark_hpm(stop_hpm);
...
ee_u32 coremark_hpm_delta(unsigned counter)
{
    return counter < 4u ? stop_hpm[counter] - start_hpm[counter] : 0u;
}
