...
for (i=0; i<=TOTAL_INT/2**(NUM_LEVELS/2) ; i++) begin : MIDDLE_FLOPS
  rvdff #(INTPRIORITY_BITS) level2_intpend_prior_reg  (.*, .din (level_intpend_w_prior_en_1[i]), .dout(l2_intpend_w_prior_en_ff[i]),  .clk(active_clk));
  rvdff #(ID_BITS)          level2_intpend_id_reg     (.*, .din (level_intpend_id_1[i]),         .dout(l2_intpend_id_ff[i]),          .clk(active_clk));
end
// LEVEL1
logic [TOTAL_INT+2:0] [INTPRIORITY_BITS-1:0] levelx_intpend_w_prior_en_2;
logic [TOTAL_INT+2:0] [ID_BITS-1:0] levelx_intpend_id_2;
    for (m=0; m<=(TOTAL_INT)/(2**(2)) ; m++) begin : COMPARE1
       if ( m == (TOTAL_INT)/(2**(2))) begin 
            assign levelx_intpend_w_prior_en_2[m+1] = '0 ;
            assign levelx_intpend_id_2[m+1]         = '0 ;
       end
       cmp_and_mux  #(
                      .ID_BITS(ID_BITS),
                      .INTPRIORITY_BITS(INTPRIORITY_BITS)) cmp_l2 (
                      .a_id(levelx_intpend_id[1][2*m]),
                      .a_priority(levelx_intpend_w_prior_en[1][2*m]),
                      .b_id(levelx_intpend_id[1][2*m+1]),
                      .b_priority(levelx_intpend_w_prior_en[1][2*m+1]),
                      .out_id(levelx_intpend_id_2[m]),
                      .out_priority(levelx_intpend_w_prior_en_2[m])) ;
...
logic [TOTAL_INT+2:0] [ID_BITS-1:0] levelx_intpend_id_3;
    for (m=0; m<=(TOTAL_INT)/(2**(3)) ; m++) begin : COMPARE2
       ...
       cmp_and_mux  #(
                      ...
                      .a_id(levelx_intpend_id_2[2*m]),
                      .a_priority(levelx_intpend_w_prior_en_2[2*m]),
                      .b_id(levelx_intpend_id_2[2*m+1]),
                      .b_priority(levelx_intpend_w_prior_en_2[2*m+1]),
                      .out_id(levelx_intpend_id_3[m]),
                      .out_priority(levelx_intpend_w_prior_en_3[m])) ;
        
 end

assign claimid_in[ID_BITS-1:0]                      =      levelx_intpend_id_3[0] ;   // This is the last level output
assign selected_int_priority[INTPRIORITY_BITS-1:0]  =      levelx_intpend_w_prior_en_3[0] ;
`else
// LEVEL0
logic [TOTAL_INT+2:0] [INTPRIORITY_BITS-1:0] level_intpend_w_prior_en_1;
logic [TOTAL_INT+2:0] [ID_BITS-1:0] level_intpend_id_1;
...
assign claimid_in[ID_BITS-1:0]                      =      level_intpend_id_3[0] ;   // This is the last level output
assign selected_int_priority[INTPRIORITY_BITS-1:0]  =      level_intpend_w_prior_en_3[0] ;
`endif
