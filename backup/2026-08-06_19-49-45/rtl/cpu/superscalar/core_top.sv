...
module core_top (
    ...
    output logic [3:0]  dbg_commit_mem_rmask_o,
    output logic [31:0] dbg_commit_mem_rdata_o,
    output logic [31:0] dbg_commit_mem_wdata_o,
    output logic [3:0]  dbg_commit_mem_wstrb_o,
    output logic [4:0]  dbg_commit_rs1_o,
    output logic [31:0] dbg_commit_rs1_rdata_o,
    output logic [4:0]  dbg_commit_rs2_o,
    output logic [31:0] dbg_commit_rs2_rdata_o,
    output logic [31:0] dbg_commit_tval_o,
    output logic [1:0]  dbg_commit_mode_o,
    output logic [31:0] dbg_commit_csr_mstatus_o,
    output logic [31:0] dbg_commit_csr_mie_o,
    output logic [31:0] dbg_commit_csr_mip_o,
    output logic [31:0] dbg_commit_csr_mtvec_o,
    output logic [31:0] dbg_commit_csr_mscratch_o,
    output logic [31:0] dbg_commit_csr_mepc_o,
    output logic [31:0] dbg_commit_csr_mcause_o,
    output logic [31:0] dbg_commit_csr_mtval_o,
    output logic [63:0] dbg_commit_csr_mcycle_o,
    output logic [63:0] dbg_commit_csr_minstret_o
);
    ...
    logic id_valid_q;
    uop_t id_uop_q, decoded_uop, issue_uop;
    ...
    logic fixed_completion_valid, fixed_completion_valid_raw;
    ...
    logic exec_load_enqueue_raw, exec_store_enqueue_raw;
    logic [31:0] exec_mem_addr_calc, exec_mem_addr_q;
    logic execute_trigger_match, memory_trigger_match;
    logic csr_access_illegal, csr_write_intent;
    ...
    logic [31:0] csr_mstatus, csr_mscratch, csr_mcause, csr_mtval;
    ...
    logic [31:0] interrupt_cause, interrupt_pc;
    ...
    function automatic logic csr_is_implemented(input logic [11:0] addr);
        unique case (addr)
            ...
            12'h3A0, 12'h3B0,
            12'h7A0, 12'h7A1, 12'h7A2, 12'h7A5,
            12'hB00, 12'hB80, 12'hB02, 12'hB82,
            12'hC00, 12'hC80, 12'hC02, 12'hC82,
            12'hF14: csr_is_implemented = 1'b1;
    ...
    assign issue_fire = issue_fire_raw && !interrupt_pending;
    assign interrupt_pc = id_valid_q ? id_uop_q.pc :
                          (fetch_valid ? fetch_head_entry.pc : irom_addr_o);
    assign take_interrupt = interrupt_pending && (scoreboard_count_q == 0) &&
                            !exec_q.valid && mem_quiescent;
    assign irq_taken_o = take_interrupt;
    assign irq_mip_o = csr_mip;

    frontend u_frontend (
        .clk(clk), .rst(rst), .redirect_valid_i(redirect_valid),
        .redirect_target_i(redirect_target), .irom_addr_o(irom_addr_o),
        .irom_ena_o(irom_ena_o), .irom_req_ready_i(irom_req_ready_i),
        .irom_resp_valid_i(irom_resp_valid_i), .irom_data_i(irom_data_i),
        .irom_resp_err_i(irom_resp_err_i),
    ...
    always_comb begin
        ...
        csr_write_intent = id_uop_q.csr_op == CSR_WRITE ||
                           ((id_uop_q.csr_op == CSR_SET ||
                             id_uop_q.csr_op == CSR_CLEAR) &&
                            (id_uop_q.csr_imm ? id_uop_q.rs1 != 0 :
                                                csr_src_value != 0));
        csr_access_illegal = id_uop_q.sys_op == SYS_CSR &&
            (!csr_is_implemented(id_uop_q.csr_addr) ||
             csr_privilege < id_uop_q.csr_addr[9:8] ||
             (csr_write_intent && id_uop_q.csr_addr[11:10] == 2'b11));
        if ((csr_privilege != 2'b11 || csr_tcontrol[3]) &&
            csr_tdata1_0[31:28] == 4'd2 &&
            ((csr_privilege == 2'b11 && csr_tdata1_0[6]) ||
             (csr_privilege == 2'b00 && csr_tdata1_0[3])) &&
            csr_tdata1_0[2] && csr_tdata2_0 == id_uop_q.pc)
            execute_trigger_match = 1'b1;
        if ((csr_privilege != 2'b11 || csr_tcontrol[3]) &&
            csr_tdata1_1[31:28] == 4'd2 &&
            ((csr_privilege == 2'b11 && csr_tdata1_1[6]) ||
             (csr_privilege == 2'b00 && csr_tdata1_1[3])) &&
            csr_tdata1_1[2] && csr_tdata2_1 == id_uop_q.pc)
            execute_trigger_match = 1'b1;
        if (execute_trigger_match) begin
            issue_uop.fu = FU_NONE;
            issue_uop.sys_op = SYS_NONE;
            issue_uop.writes_rd = 1'b0;
            issue_uop.serialize = 1'b1;
            ...
            issue_uop.serialize = 1'b1;
            issue_uop.exception_valid = 1'b1;
            issue_uop.exception_cause = 5'd2;
            issue_uop.exception_tval = id_uop_q.instr;
        end
    end
    ...
    fixed_execute u_fixed_execute (
        ...
        .completion_valid_o(fixed_completion_valid_raw),
        .completion_o(fixed_completion_raw),
        ...
        .branch_kind_o(branch_kind_c), .load_enqueue_o(exec_load_enqueue_raw),
        .store_enqueue_o(exec_store_enqueue_raw), .mem_addr_o(exec_mem_addr_calc)
    );

    always_comb begin
        fixed_completion = fixed_completion_raw;
        fixed_completion_valid = fixed_completion_valid_raw;
        exec_load_enqueue = exec_load_enqueue_raw;
        exec_store_enqueue = exec_store_enqueue_raw;
        memory_trigger_match = 1'b0;
    ...
    recovery_ctrl u_recovery_ctrl (
        ...
        .interrupt_valid_i(take_interrupt), .interrupt_cause_i(interrupt_cause),
        .branch_miss_i(branch_miss),
    ...
    assign csr_trap_valid = take_interrupt ||
                            (commit_fire && commit_entry.exception_valid);
    assign csr_mret_valid = commit_normal && commit_entry.sys_op == SYS_MRET;
    assign csr_trap_cause = take_interrupt ? interrupt_cause :
                            ((commit_entry.sys_op == SYS_ECALL) ?
                             {30'd0, csr_privilege} + 32'd8 :
                             {27'd0, commit_entry.exception_cause});
    ...
    csr_file u_csr (
        ...
        .trap_pc_i(take_interrupt ? interrupt_pc : commit_entry.pc),
    ...
    commit_trace_probe u_commit_trace_probe (
        ...
        .issue_src1_i(((issue_uop.fu == FU_LOAD) ||
                       (issue_uop.fu == FU_STORE)) ? src1_mem_value : src1_value),
        .issue_src2_i(src2_value),
        .branch_resolve_i(branch_resolve_valid_c),
        .branch_actual_next_i(branch_actual_next_c), .exec_i(exec_q),
        .exec_mem_addr_i(exec_mem_addr_q), .cycle_i(csr_mcycle),
        .commit_count_i(csr_minstret),
        .csr_privilege_i(csr_privilege), .csr_mstatus_i(csr_mstatus),
        .csr_mie_i(csr_mie), .csr_mip_i(csr_mip),
        .csr_mscratch_i(csr_mscratch), .csr_mcause_i(csr_mcause),
        ...
        .commit_mem_rmask_o(dbg_commit_mem_rmask_o),
        .commit_mem_rdata_o(dbg_commit_mem_rdata_o),
        .commit_mem_wdata_o(dbg_commit_mem_wdata_o),
        .commit_mem_wstrb_o(dbg_commit_mem_wstrb_o),
        .commit_rs1_o(dbg_commit_rs1_o),
        .commit_rs1_rdata_o(dbg_commit_rs1_rdata_o),
        .commit_rs2_o(dbg_commit_rs2_o),
        .commit_rs2_rdata_o(dbg_commit_rs2_rdata_o),
        .commit_tval_o(dbg_commit_tval_o),
        .commit_mode_o(dbg_commit_mode_o),
        .commit_csr_mstatus_o(dbg_commit_csr_mstatus_o),
        .commit_csr_mie_o(dbg_commit_csr_mie_o),
        .commit_csr_mip_o(dbg_commit_csr_mip_o),
        .commit_csr_mtvec_o(dbg_commit_csr_mtvec_o),
        .commit_csr_mscratch_o(dbg_commit_csr_mscratch_o),
        .commit_csr_mepc_o(dbg_commit_csr_mepc_o),
        .commit_csr_mcause_o(dbg_commit_csr_mcause_o),
        .commit_csr_mtval_o(dbg_commit_csr_mtval_o),
        .commit_csr_mcycle_o(dbg_commit_csr_mcycle_o),
        .commit_csr_minstret_o(dbg_commit_csr_minstret_o)
    );
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            if (full_flush) begin
                ...
                exec_q.valid <= issue_fire && !issue_uop.exception_valid && issue_uop.fu != FU_SYSTEM;
                if (issue_fire && !issue_uop.exception_valid && issue_uop.fu != FU_SYSTEM) begin
                    exec_q.trans_id <= issue_ptr_q;
                    exec_q.uop <= issue_uop;
                    exec_q.op1 <= ((issue_uop.fu == FU_LOAD) ||
