...
module biriscv_cpu_core #(
  ...
  logic [1:0] core_event_count;
  logic core_interrupt_taken;
  logic [63:0] order_q;
  logic data_fault_q;

  // Slot 0 can carry a synchronous exception; slot 1 is a normal WB event.
  // Keep performance accounting independent of the single-slot trace payload.
  always_comb begin
    retire_count_o = {1'b0, core_commit_valid &&
                      (core_commit_exception == 0)} +
                     {1'b0, core_commit_slot1_valid};
    core_event_count = {1'b0, core_commit_valid} +
                       {1'b0, core_commit_slot1_valid};
  end
