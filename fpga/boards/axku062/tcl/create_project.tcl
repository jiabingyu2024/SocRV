...
read_socrv_filelist $repo_dir [file join $repo_dir sim filelists fpga_axku062.f]
...
read_xdc [list [file join $repo_dir fpga boards axku062 constraints pins.xdc]]
read_xdc [list [file join $repo_dir fpga boards axku062 constraints clocks.xdc]]
read_xdc [list [file join $repo_dir fpga boards axku062 constraints cdc.xdc]]

set_property top fpga_top [current_fileset]
set clock_mult [expr {[info exists ::env(SOCRV_CLOCK_MULT)] ? $::env(SOCRV_CLOCK_MULT) : "5.0"}]
set core_divide [expr {[info exists ::env(SOCRV_CORE_DIVIDE)] ? $::env(SOCRV_CORE_DIVIDE) : "10.0"}]
set peripheral_divide [expr {[info exists ::env(SOCRV_PERIPHERAL_DIVIDE)] ? $::env(SOCRV_PERIPHERAL_DIVIDE) : 20}]
set core_hz [expr {[info exists ::env(SOCRV_CORE_HZ)] ? $::env(SOCRV_CORE_HZ) : 100000000}]
