...
hash 和 Memory Map hash，任何一项变化都会使 `make isa-data-check` 失败，
要求重新生成，不允许继续使用旧地址布局的 MEM。

当前生成集共 86 项：

```text
rv32ui  41    final-base；current gate 另排除 fence_i
rv32mi  16    final-base 的 Machine Mode/CSR/counter
rv32um   8    final-base 的 M extension
rv32uf  11    single-precision candidate
rv32ud  10    double-precision candidate
```

固定版本上游 RV32UD Makefrag 排除 `move`，项目选择文件保留原因；`ma_data`
根据 misaligned Trap 策略排除。依赖锁同时校验 riscv-tests 主仓库和其
`riscv-test-env` 子模块。
...
make sim-isa                 # current
make sim-isa-final-base      # RV32UI + RV32MI + RV32UM
make sim-isa-fp-single       # RV32UF candidate
make sim-isa-fp-double       # RV32UD candidate
make sim-isa-final           # base + selected FP; pending 时阻塞
```

`make sim-required` 仍使用 `current` gate，再顺序运行 smoke、RT-Thread、
RT-Thread + 1 轮 CoreMark 正确性和 RT-Thread + 10 轮 CoreMark 简单性能
统计。待正式核完成后，发布/签核门应显式增加 `sim-isa-final`，不能把
candidate gate 或仅生成数据视为 CPU 已通过。
