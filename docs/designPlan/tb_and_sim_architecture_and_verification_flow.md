...
ISA 数据和 CPU 验收分离，避免 demo core 的能力限制最终设计：

| Gate | 套件 | 当前状态 | 用途 |
| --- | --- | --- | --- |
| `current` | demo core 已实现的 RV32UI 子集 | 可运行 | 日常快速防回归，`make sim-isa` |
| `final-base` | RV32UI + RV32MI + RV32UM | 数据已生成，正式核实现后运行 | RV32IM、Machine Mode、CSR/counter、`fence.i` |
| `fp-single` | RV32UF | 候选数据已生成 | 选择单精度 F 时的附加门 |
| `fp-double` | RV32UD | 候选数据已生成 | 选择双精度 FD 时的附加门 |
| `final` | `final-base` + 选定的浮点门 | 当前阻塞 | 最终 CPU 签核入口 |

对应入口为 `make isa-gates`、`make sim-isa-final-base`、
`make sim-isa-fp-single`、`make sim-isa-fp-double` 和
`make sim-isa-final`。浮点仍为 `pending` 时，最后一个入口必须在启动 RTL
模型前失败；选择 F/FD 后由合同生成唯一组合。

RV32MI 验证 Machine Mode 特权行为，不表示存在 “MI 扩展”。最终 base gate
除整数指令外，还要求检查 illegal instruction、`ecall`/`ebreak`、`mret`、
`mstatus/mie/mip/mtvec/mepc/mcause/mtval`、中断优先级、`cycle/instret`
及其 RV32 高半 CSR，还包含 `pmpcfg0/pmpaddr0` 的最小 PMP 行为。
RV32UM 覆盖乘除法边界，尤其是除零和有符号溢出。
浮点门还必须覆盖 FP 寄存器写回、rounding mode、exception flags 和
`mstatus.FS`；若使用 DiffTest，reference state 也应扩展这些字段。
